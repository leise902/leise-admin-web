...
<script>
import { queryUserList, addUser } from '@/api/user.js'
...
export default {
                ...
                render: (h, params) => {
                    return h ('div', [
                        ...
                        h('Button', {
                            ...
                            on: {
                                click: () => {
                                    this.handleRenderEdit(params.row.userId)
                                }
                            }
                        }, '编辑')
                    ])
    ...
    methods: {
        ...
        handleRenderAdd () {
            let vm = this
            userRender.addUserInfo(vm)
        },

        handleRenderView (userId) {
            let vm = this
            userRender.viewUserInfo(vm, userId)
        },

        handleRenderEdit (userId) {
            let vm = this
            userRender.editUserInfo(vm, userId)
        }
    },
    mounted () {
        this.init();
