...
<script>
import { queryModulePage } from '@/api/module.js'
...
export default {
            ...
            return h('div', [
              h(
                'Button',
                {
                  ...
                  on: {
                    click: () => {
                      this.handleRenderConfigModuleEnv(params.row.moduleInfoId)
                    }
                  }
                },
                '环境配置'
  ...
  methods: {
    ...
    handleRenderConfigModuleEnv(data) {
      let vm = this
      moduleRender.configModuleEnv(vm, data)
    }
  }
}
</script>
